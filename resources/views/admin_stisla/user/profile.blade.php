...
@section('page')
    <div class="card has-shadow">
        <div class="card-body" style="height:200px;padding-top:70px;background: rgb(48,58,203);background: linear-gradient(90deg, rgba(48,58,203,1) 0%, rgba(182,50,102,1) 100%);">
            <img src="{!! $profile->avatar ?? '' !!}" class="circle" style="border-radius: 40px" width="80" height="80">&nbsp;
            &nbsp;
            <span style="color: #ffffff;font-size: 1.3em;">{!! $profile->username ?? '' !!}</span>
        </div>
    </div>
            ...
            <div class="card has-shadow">
                ...
                <div class="card-body">
                    <ul style="font-size: .8em;">
                        <li><label>{{ trans('admin.name') }}</label><span @if(isRtl()) class="float-right" @else class="float-left" @endif style="color: gray">{{{ $profile->name ?? '' }}}</span></li>
                        <hr>
                        <li><label>{{ trans('admin.City') }}</label><span @if(isRtl()) class="float-right" @else class="float-left" @endif style="color: gray">{{{ $profile->city ?? '' }}}</span></li>
                        <hr>
                        <li><label>{{ trans('admin.gender') }}</label><span @if(isRtl()) class="float-right" @else class="float-left" @endif style="color: gray">@if($profile->gender == 'm'){{ trans('admin.Man') }}@else{{ trans('admin.Female') }}@endif</span></li>
                        <hr>

...
@section('title',trans('admin.submit_a_review_request'))
@section('page')
    <div class="card has-shadow" style="background-color: #0F6FD5;">
        <div class="card-body text-center">
            <div class="row">
                ...
                <div class="col-12 col-md-4 text-white d-flex flex-column justify-content-center align-items-center">
    ...
    @foreach($revision->messages as $message)
        <div class="card has-shadow mt-2">
            <div class="card-header bordered d-flex justify-content-between align-items-center no-actions @if($message->user_id == $project->user_id) bg-danger @elseif($message->user_id == $project->contractor_id) bg-warning @else bg-blue @endif">
                {{ $message->user->name ?? '' }}&nbsp;@if($message->user_id != $project->user_id && $message->user_id != $project->contractor_id)({{ trans('admin.supporter') }})@endif
                <span>{{ getJDate($message->created_at) }}</span>
            </div>
    ...
    @endforeach
    @if($revision->mode != 'done')
    <div class="card has-shadow">
        <div class="card-header bordered no-actions"><h4>{{ trans('admin.post_reply') }}</h4></div>
        <form method="post" enctype="multipart/form-data" action="/user/project/revision/reply/store/{{ $revision->id }}">
            <div class="card-body">
                <div class="form-group">
                    <label>{{ trans('admin.text') }}</label>
                    <textarea class="summernote" name="message" rows="10" required></textarea>
                </div>
                <div class="my-2">
